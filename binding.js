...
export function assertAgentKey(agentKey) {
  if (typeof agentKey !== 'string' || !AGENT_KEY.test(agentKey)) {
    throw new Error('agent-model-binding: agent_key must match [A-Za-z0-9][A-Za-z0-9_-]*')
  }
  return agentKey
}

export function parseBindingHeader(text, filename = '<binding>') {
  if (typeof text !== 'string') throw new TypeError('agent-model-binding: binding content must be text')
...
export async function loadBinding(bindingsDir, agentKey) {
  ...
  try {
    ...
    throw new Error(`agent-model-binding: failed to read ${filename}`, { cause: error })
  }
...
export async function assertRegisteredModel(llm, binding) {
  ...
  if (!providers.some(entry => entry.id === binding.provider)) {
    throw new Error(
      `agent-model-binding: provider ${JSON.stringify(binding.provider)} from ${binding.filename} is not registered`,
    )
  }
  const models = await llm.listModels(binding.provider)
  if (!models.some(entry => entry.id === binding.model)) {
    throw new Error(
      `agent-model-binding: model ${JSON.stringify(binding.model)} is not registered under provider `
