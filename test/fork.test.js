...
async function harness(files = {}, config = {}) {
  const bindingsDir = await mkdtemp(join(tmpdir(), 'evo-subagent-fork-'))
  ...
  createApply(value => value)(ctx, { bindingsDir, provider: 'fork', toolName: 'evo_subagent', maxDepth: 3, ...config })
